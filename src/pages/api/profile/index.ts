...
function sanitizeUrl(v: unknown, max = 500): string {
  ...
  // Accepte uniquement https:// ou chaîne vide
  return s === '' || s.startsWith('https://') ? s : ''
}

function sanitizeTheme(raw: Record<string, unknown>): Record<string, unknown> {
  return {
    ...raw,
    // Couleurs
...
export const PUT: APIRoute = wrapApiHandler<ProfileUpdateBody>(
  async ({ profile, body }: ApiHandlerContext<ProfileUpdateBody>) => {
    ...
    if (body.theme !== undefined) {
      ...
      update.theme = toJson(sanitizeTheme(body.theme))
    }
