...
function adjustForContrast(color: {l: number; c: number; h: number}, bg: string, mode: 'dark' | 'light') {
  // On ne passe plus "mode" à culori
  let c = { ...color };
  let attempts = 0;
...
export function generateFullPalette(seedString: string, mode?: 'dark' | 'light') {
  ...
  let primary = adjustForContrast({ l: baseL, c: base.c!, h: primaryHue }, background, finalMode);
  let secondary = adjustForContrast({ l: baseL, c: base.c! * 0.9, h: secondaryHue }, background, finalMode);
  let accent = adjustForContrast({ l: baseL, c: base.c! * 1.1, h: accentHue }, background, finalMode);
  let gradient = adjustForContrast({ l: baseL, c: base.c!, h: gradientHue }, background, finalMode);
