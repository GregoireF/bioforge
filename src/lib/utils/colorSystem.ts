...
function adjustForContrast(color: culori.OklchColor, bg: string, mode: 'dark'|'light') {
  let c = {...color};
  let attempts = 0;
  while(culori.contrast(culori.formatHex(c), bg) < 4.5 && attempts<20){
    c.l += mode==='dark'? 0.02 : -0.02;
    c.c += mode==='dark'? 0.01 : 0.01;
...
export function generateFullPalette(seedString: string, mode?: 'dark'|'light') {
  ...
  const finalMode = mode || detectModeFromHex(seedHex); // auto detect if not provided
  ...
  let primary = adjustForContrast({l: baseL, c: base.c!, h: primaryHue, mode:'oklch'}, background, finalMode);
  let secondary = adjustForContrast({l: baseL, c: base.c!*0.9, h: secondaryHue, mode:'oklch'}, background, finalMode);
  let accent = adjustForContrast({l: baseL, c: base.c!*1.1, h: accentHue, mode:'oklch'}, background, finalMode);
  let gradient = adjustForContrast({l: baseL, c: base.c!, h: gradientHue, mode:'oklch'}, background, finalMode);
